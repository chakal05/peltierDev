				...
				<v-card height="700">
					<v-toolbar color="teal darken-4" dark>
						...
						<v-toolbar-title class="newMessage">
							<v-btn @click="getContacts" color="teal darken-4"
								>Contacts</v-btn
							>
						</v-toolbar-title>
					</v-toolbar>
		...
		methods: {
			...
			async loadMessages() {
				await axios
					.get('/messages', {
						params: {
							id: JSON.parse(localStorage.getItem('userToken'))
								.tokenUserId,
							data: 'messages',
						},
					})
			...
			async save() {
				...
				await axios
					.post('/messages', {
						name:  JSON.parse(localStorage.getItem('userToken'))
								.tokenUserName,
						to: this.user_to,
						from:JSON.parse(localStorage.getItem('userToken'))
								.tokenUserId,
						time: timestamp,
