	...
	export default {
		data: () => ({
			...
			telephoneRules: [
				...
				(v) =>
					(v && v.length >= 8) ||
					'Phone number should contain 8 numbers min',
				(v) =>
					!isNaN(v) ||
		...
		methods: {
					...
					.then((response) => {
						if (response.data.length === 0) {
							...
							let displayHours = baseHours
								...
								.concat(
									response.data.filter((x) => !baseHours.includes(x))
								);
							if (!displayHours[0]) {
								//if fully booked
			...
			async register() {
				...
				await axios
					.post('/reservations', {
						...
						rank: this.rank,
						addedBy: JSON.parse(localStorage.getItem('userToken'))
							.tokenUserName,
					})
					.then((response) => {
			...
			async changeItem() {
				...
				await axios
					...
					.catch(() => {
						this.error =
							'There was an error with the editing of the item';
					});
			},
